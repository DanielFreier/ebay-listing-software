...
exports.receivenotify = function(req, res) {
    ...
    req.on('end', function() {
        ...
        parser.parseString(xml, function (err, json) {
            ...
            /* GetItemTransactions */
            if (rootname == 'GetItemTransactionsResponse') {
                mongo(function(db) {
                    db.collection('users', function(err, collection) {
                        collection.findOne({'userids2.username': userid}, function(err, user) {
                            if (!user) return
                            ...
                            db.collection('items.' + user._id, function(err, itemcoll) {
                                itemcoll.findOne(
                                    ...
                                    function(err, item) {
                                        if (!item) return;
                                        if (!body.hasOwnProperty('TransactionArray')) return;
                                        
                                        var git = require('./ebayapi/GetItemTransactions');
                                        
                                        var transactions = body.TransactionArray.Transaction;
                                        ...
                                        transactions.forEach(
                                            function(transaction) {
                                                git.upsert_transaction(transaction,
                                                                       itemcoll,
                                                                       body.Item.ItemID);
                                            }
                                        );
