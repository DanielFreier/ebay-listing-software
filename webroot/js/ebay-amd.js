define(function() {
    ...
    var ebayjs = {
        ...
        setformelements: function(item)
        {
            ...
            /* ListingType */
            this.setformelements_listingtype(item);
            ...
            if (item.opt && item.opt.ScheduleType) {
                $('input[name="mod.ScheduleTime.date"]', idstr).show();
                $('input[name="mod.ScheduleTime.time"]', idstr).show();
            }	
        ...
        setformelements_listingtype: function(item) {
            
	        var id = item.id;
            var idstr = '#' + item.id + '-detail';
            
            if (item.mod.ListingType == 'FixedPriceItem') {
                $('label[for="StartPrice"]', idstr).html('Buy It Now price');
                $('.buyitnowprice', idstr).hide();
            } else {
                $('label[for="StartPrice"]', idstr).html('Starting price');
                $('.buyitnowprice', idstr).show();
            }
            
            return;
            
        }, //setformelements_listingtype
        ...
        showformvalues: function(item) {
	        ...
	        */
	        ...
	        /* checkbox */
	        $.each($('input[type="checkbox"][name^="mod"], input[type="checkbox"][name^="opt"]', detail), function(i, form) {
		        var formname = $(form).attr('name');
		        if (formname == null) return;
		        formname = "['" + formname.replace(/\./g, "']['") + "']";
		        try {
            ...
            if (item.mod.BuyerRequirementDetails) {
                if (item.mod.BuyerRequirementDetails.MinimumFeedbackScore) {
                    $('input[name="mod.BuyerRequirementDetails.MinimumFeedbackScore.checkbox"]', detail)
                        .replaceWith('<i class="icon-ok green"></i>');            
                }
                if (item.mod.BuyerRequirementDetails.MaximumItemRequirements
                    && item.mod.BuyerRequirementDetails.MaximumItemRequirements.MinimumFeedbackScore) {
                    $('input[name="mod.BuyerRequirementDetails.MaximumItemRequirements.MinimumFeedbackScore.checkbox"]', detail)
                        .replaceWith('<i class="icon-ok green"></i>');            
                }
            }
        ...
        fillformvalues: function(item) {
            ...
            */
	        ...
	        // checkbox
	        $.each($('input[type="checkbox"][name^="mod"]', '#' + id + '-detail'), function(i, form) {
		        var formname = $(form).attr('name');
		        formname = "['" + formname.replace(/\./g, "']['") + "']";
		        try {
			        eval("var tmpvalue = item"+formname);
            ...
            if (item.mod.BuyerRequirementDetails) {
                if (item.mod.BuyerRequirementDetails.MinimumFeedbackScore) {
                    $('input[name="mod.BuyerRequirementDetails.MinimumFeedbackScore.checkbox"]', idstr)
                        .prop('checked', true);
                }
                if (item.mod.BuyerRequirementDetails.MaximumItemRequirements
                    && item.mod.BuyerRequirementDetails.MaximumItemRequirements.MinimumFeedbackScore) {
                    $('input[name="mod.BuyerRequirementDetails.MaximumItemRequirements.MinimumFeedbackScore.checkbox"]', idstr)
                        .prop('checked', true);
                }
            }
        ...
        clearrows: function() {
            ...
            $('#items > .rowbid').remove();
            $('#items > .row-2nd-offer-form').remove();
            
            $('#items > .row-transaction').remove();
            $('#items > .row-transaction-message').remove();
            
            $('#items > .row-dispute').remove();
            $('#items > .row-reply-dispute-form').remove();
            
            $('#items > .rowmembermessages').remove();
            $('#items > .rowmembermessages-body').remove();
